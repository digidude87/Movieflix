...
function mcImgSlider(i) {
	...
	}, P = document, J = window.requestAnimationFrame, V = window.cancelAnimationFrame, nb = [
			"ms", "webkit" ], v = "", db = 0; db < nb[d] && !J; ++db) {
		v = nb[db];
		J = window[v + "RequestAnimationFrame"];
		V = window[v + "CancelAnimationFrame"];
		v = "-" + v + "-"
	}
	var yb = function() {
		var b = U(P, "head");
		...
		if (a)
			if (typeof a.insertRule != "undefined") {
				var b = "@" + v + "keyframes jisSpinner {from{" + v
						+ "transform:rotate(0deg);} to{" + v
				...
				var d = "#"
						...
						+ "::before {"
						+ v
						+ "animation:jisSpinner 0.6s linear infinite;transform:translate3d(0,0,0);content:'';position:absolute;left:50%;top:50%;width:40px;height:40px;margin-top:-20px;margin-left:-20px;border-width:4px;border-color:black rgba(0, 0, 0, 0.8) rgba(255, 255, 255, 0.8) rgba(255, 255, 255, 0.8);border-style:solid;border-radius:50%;}";
				a.insertRule(d, 0)
			} else
				a.addRule("#" + i.sliderId + " .bgLayer",
						"position:absolute;width:100%;height:100%;")
	};
	sb();
	var c = "style", H = "display", Eb = "visibility", r = "width", q = "height", O = "top", B = "background", p = "undefined", Fb = "marginLeft", F = "appendChild", l = "parentNode", k = "nodeName", S = "innerHTML", bb = "offsetWidth", u = setTimeout, z = clearTimeout, w = "indexOf", N = "setAttribute", ab = "removeChild", L = function() {
		this.d = [];
		this.b = null
	}, zb = function() {
		...
		if (a[w]("Opera") != -1)
			b = 400;
		return b
	}, T = zb() < 9, Z = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i
			.test(navigator.userAgent), K = function(a, b) {
		if (a) {
			a.o = b;
			if (T)
	...
	L.prototype = {
		...
		m : function(h, d, g, c) {
			for (var b = [], i = g - d, j = g > d ? 1 : -1, f = Math
		...
		s : function(c, b) {
			...
			var a, d = {};
			for (a in c)
				d[a] = typeof b[a] !== p ? b[a] : c[a];
			return d
		}
	};
	var G = new L, fb = function() {
		G.d = [];
		z(m);
		z(R);
		m = R = null
		...
		c : 0,
		b : 0
	}, a, f, o, s, D, A, m, R, x, M, W, e, E, j = null, vb = function() {
		...
		if (f)
			a.m = Math.ceil(f.offsetHeight * a.g / f[bb]);
		hb(i.effect);
		a.n = function() {
			var b;
			if (a.a.p == -1)
	...
	}, qb = [ "$1$2$3", "$1$2$3", "$1$24", "$1$23", "$1$22" ], jb = function() {
		if (b.b != 2) {
			b.b = 1;
			z(m);
			m = null
		...
		while (a < e.length) {
			c = e[a][g] == "lazyImage" || e[a][h]("data-src")
					|| e[a][g][w](" video") > -1 && typeof McVideo != p;
			if (c) {
				b = 1;
				break
			}
		...
		return a[l][h]("data-autovideo") == "true"
	...
	kb.prototype = {
		...
		e : function(j) {
			var a = e[j], m = 0;
			if (a[k] == "A" && a[g][w]("lazyImage") == -1 || a[k] == "DIV"
					&& a[g] == "video") {
				a = t(a)[0];
				m = 1
			}
		...
		v : function() {
			...
			return (new Function(
					...
					.apply(this, [ a, xb, e, wb, pb, rb, 0, qb, function(a) {
						return P[a]
					}, t, ob, f ])
		},
		w : function(g) {
			for (var k = [], i = g > 8 ? o : Math.round(o / a.f), l = g > 8 ? 1
					: a.f, f = 0; f < l; f++) {
				var j = C("mcSlc"), e = j[c];
				e.left = i * f + "px";
				e[r] = (f == a.f - 1 ? o - i * f : i) + "px";
				e[q] = "0px";
				e[B] = 'url("' + b.e[h]("src") + '") no-repeat -' + f * i
						+ "px 0%";
				if (g == 10)
		...
		x : function() {
			for (var k = [], j = Math.round(o / a.g), i = Math.round(s / a.m), g = 0; g < a.m; g++)
				for (var f = 0; f < a.g; f++) {
					...
					e[r] = d.w + "px";
					e[q] = d.h + "px";
					e[B] = 'url("' + b.e[h]("src") + '") no-repeat -' + f * j
							+ "px -" + g * i + "px";
					e.position = "absolute";
					K(d, 0);
					E[F](d);
