var moviejson = {
	"movies" : [
			...
			},
			{
				...
				"Writer" : "Mark L. Smith (screenplay), Alejandro GonzÃ¡lez IÃ±Ã¡rritu (screenplay), Michael Punke (based in part on the novel by)",
			...
			},
			{
				...
				"Actors" : "Daniel Craig, Christoph Waltz, LÃ©a Seydoux, Ralph Fiennes",
			...
			},
			{
				...
				"Actors" : "Matt Damon, Robin Williams, Ben Affleck, Stellan SkarsgÃ¥rd",
...
function jsonParser() {
	for (var i = 0; i < moviejson.movies.length; i++) {
		var movie = moviejson.movies[i];
		var genres = movie.Genre.split(",");
		console.log(genres);
		for(var j=0;j<genres.length;j++) {
			if(genres[j].trim()=="Action")
				action.push(movie.Poster);
			if(genres[j].trim()=="Comedy")
				comedy.push(movie.Poster);
			if(genres[j].trim()=="Drama")
				drama.push(movie.Poster);
			if(genres[j].trim()=="Horror") 
				horror.push(movie.Poster);
			if(genres[j].trim()=="Romance")
				romance.push(movie.Poster);
		}
	}
}
